...
class CmdVelCombiner(Node):
    def __init__(self):
        ...
        self.gain = 15.0 #K gain for velocity scaling based on curvature
        self.curvature = 0.0
    ...
    def timer_callback(self):
        ...
        combined = Twist()
        combined.linear.x = self.scale_velocity()  # longitudinal from PID
        combined.angular.z = self.pure_pursuit_cmd.angular.z  # steering from Pure Pursuit

        self.get_logger().info(f"Linear: {combined.linear.x:.2f}, Angular: {combined.angular.z:.2f} | Curvature: {self.curvature:.4f}")
