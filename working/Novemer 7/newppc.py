...
class PurePursuitController(Node):
    def __init__(self):
        ...
        self.declare_parameter('lookahead_min', 2.4)
        self.declare_parameter('lookahead_gain', 1.1)
        ...
        self.steer_time_pub = self.create_publisher(PointStamped, '/steering_time', 10)

        self.create_timer(0.1, self.purepursuit_callback) #20Hz
    ...
    def compute_vref(self, curvature):
        v_max = 6.25 # max speed was found at 6.25
        k_v = 3.0 #tuning gain for speed reduction (higher value leads to slower speed at turns)

        v_ref = v_max / (1.0 + k_v * abs(curvature))

        msg = Float64()
        msg.data = v_ref
        self.v_ref_pub.publish(msg)
        return v_ref
    ...
    def purepursuit_callback(self):
        ...
        #self.get_logger().info(f"LD: {ld:.2f} m | MIN_LD: {self.ld_min:.2f} r/s | Velocity: {self.vehicle_v:.4f} m/s")
        ...
        steer_time.point.y = time.time()
        self.steer_time_pub.publish(steer_time)
        #self.get_logger().info(f"Yaw Rate: {angular_velocity:.4f}")
