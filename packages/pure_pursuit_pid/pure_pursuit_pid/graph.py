...
class VelocityPlotter(Node):
    def __init__(self):
        ...
        # Buffers
        self.max_len = 300  # store last 300 samples (~30 seconds @ 10Hz)
        ...
        # Timer to record data from ROS
        self.create_timer(0.1, self.record_data)  # 10Hz

        # Initialize plot in a separate thread
        threading.Thread(target=self.init_plot, daemon=True).start()
    ...
    def init_plot(self):
        ...
        self.line_target, = self.ax1.plot([], [], label='Target Velocity', color='green')
        ...
        plt.tight_layout()
        plt.show(block=True)  # blocks only the plot thread
    ...
    def update_plot(self, frame):
        ...
        # Dynamic y-axis for velocities
        y_max = max(max(self.actual_velocities, default=1.0), max(self.target_velocities, default=1.0)) * 1.2
