...
class CmdVelCombiner(Node):
    ...
    def pp_callback(self, msg):
        ...
        self.get_logger().debug(f"Recieved angle {msg.angular.z}")

    #Callback function to set the msg from pid_cmd_vel to the class variable set in the beginning
    def pid_callback(self, msg):
        self.pid_cmd = msg
        self.get_logger().debug(f"Recieved Speed {msg.linear.x}")
    ...
    def timer_callback(self):
        ...
        #combined.linear.x = 100.0 #Constant Velocity
        ...
        #self.get_logger().info(f"Sent Angle: {combined.angular.z:.2f} | Speed: {combined.linear.x:.2f}")
